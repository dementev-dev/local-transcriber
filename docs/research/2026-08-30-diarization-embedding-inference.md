...
Минимальная доступная CPU-матрица для обоих непосредственных кандидатов:

| Класс | Конкретный доступный ориентир | Зачем |
|---|---|---|
| Консервативный x86 baseline: AVX2, без VNNI/AVX-512 | Intel Core i7-6820HQ, 4C/8T | Проверить нижнюю границу INT8 и конкуренцию на старом CPU. |
| Современный x86 baseline: VNNI + AVX-512 | AMD Ryzen 7 8845H из предыдущих замеров, с обязательной фиксацией `lscpu`/CPUID | Проверить специализированный INT8 kernel и масштабирование на 8C/16T. |

Эти две машины доступны для воспроизводимых прогонов; ни одна из них не является
целевым SKU проекта. Ноутбук с Intel Core i5 11-го поколения - лишь пример
распространенного пользовательского класса и не обязательная приемочная машина.
Для первого отбора кандидата двух ориентиров достаточно; на любой дополнительной
машине нужно записывать точную модель и CPUID. Переносить полученный коэффициент
ускорения на весь x86 нельзя.
