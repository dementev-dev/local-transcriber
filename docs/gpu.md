...
### VAD threshold — проверено, не помогает

Гипотеза: дефолтный Silero VAD `threshold=0.5` мог обрезать тихие реплики менти до
`Mm-hmm`. Проверено прогоном `scripts/parakeet_vad_tune.py` на тех же отрывках с
`threshold ∈ {0.5, 0.3, 0.2}`:

| Отрывок | threshold | слов | Mm-hmm-сегментов |
|---|---|---|---|
| artur-intro | 0.5 (default) | 2024 | 10 |
| artur-intro | 0.3 | 2000 | 9 |
| artur-intro | 0.2 | 1931 | 10 |
| mar15-mid | 0.5 (default) | 2531 | 6 |
| mar15-mid | 0.3 | 2504 | 8 |
| mar15-mid | 0.2 | 2366 | 6 |

Вывод: снижение threshold **уменьшает** объём транскрипта (длинные склеенные
сегменты → Parakeet хуже декодирует разреженную речь), и **не снижает частоту
filler-сегментов**. Проблема редукции реплик — inherent для модели на
тихом русском диалоге, а не артефакт VAD. Дефолт 0.5 остаётся оптимальным.
