...
где доступны uv и PyPI; команда установки явно запрашивает Python 3.13, поэтому
uv найдёт или скачает нужный интерпретатор. Закрытых контуров и оффлайн-зеркал
среди пользователей нет.

Рабочая версия фиксируется файлом `.python-version`. Floor разрешает любой
интерпретатор от 3.13 и выше, а пин делает окружение одинаковым на разных
машинах — иначе расхождение вылезает именно тогда, когда что-то отлаживаешь.
