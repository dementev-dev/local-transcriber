...
- Аппаратно-зависимое автоматическое включение диаризации.
...
- `--diarize` включает диаризацию, `--no-diarize` явно выключает. Булев ключ
  `diarize` в `.transcriber.toml` задаёт значение по умолчанию; без него
  диаризация выключена. CLI-флаги имеют приоритет над конфигом.
- `--speakers N`, где `N >= 1`, задаёт известное число участников и сам включает
  диаризацию. Без него число кластеров определяется автоматически. Сочетание
  `--speakers N` с `--no-diarize` считается ошибкой использования.
