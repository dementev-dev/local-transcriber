...
по-прежнему ждет длинных негативных примеров и проверки производительности на
нескольких доступных классах CPU.

Под CPU-дефолтом в этом разделе понимается программный профиль: backend, модель
и `compute_type`, выбранные без явных аргументов пользователя. Целевого
процессора и обязательного эталонного SKU у проекта нет.
...
- использовать минимум два доступных x86-ориентира: консервативный baseline на
  Intel Core i7-6820HQ и современный baseline на AMD Ryzen 7 8845H;
- для каждого прогона записывать точный SKU, физические ядра и логические
  потоки, CPUID/ISA, объем RAM, power mode и фактическое число потоков;
- Ryzen использовать как основную машину для итераций и как современный
  контролируемый baseline. Результат относится только к зафиксированной
  конфигурации;
- при доступности отдельно собирать результаты на других x86-ноутбуках и Apple
  Silicon как данные о переносимости; в контролируемую матрицу входят два
  предыдущих x86 baseline;
...
- Если E2E RNN-T сохраняет качество и приемлемую скорость на выбранной
  CPU-матрице - сделать его русским CPU-дефолтом, OpenVINO оставить явной
  опцией.
...
пользователи работают на ноутбуках разных поколений и производителей.
Среди них встречаются и более скромные машины уровня Intel Core i5 11-го
поколения. GigaAM v3 остается рабочей явной альтернативой для русского, но
выбор безопасного auto/default требует сравнительного бенчмарка.
