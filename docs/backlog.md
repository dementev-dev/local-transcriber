...
по-прежнему ждёт длинных негативных примеров и проверки производительности на
...
- для каждого прогона записывать точный SKU, физические ядра и логические
  потоки, CPUID/ISA, объём RAM, power mode и фактическое число потоков;
- Ryzen использовать как основную машину для итераций и как современный
  контролируемый baseline, а не переносить его результат на остальные CPU;
- при доступности отдельно собирать результаты на других x86-ноутбуках и Apple
  Silicon как данные о переносимости. Такие внешние прогоны не заменяют
  контролируемую матрицу;
...
Встречаются и машины уровня Intel Core i5 11-го поколения, но это пример
распространенной конфигурации, а не целевой SKU. GigaAM v3 остается рабочей
явной альтернативой для русского, но выбор безопасного auto/default требует
сравнительного бенчмарка.
